...
def train_model():
    ...
    pd={'n_estimators':randint(200,600),'max_depth':randint(3,8),
    ...
    xgb=XGBClassifier(objective='binary:logistic',eval_metric='auc',
                      use_label_encoder=False,tree_method='hist',random_state=RANDOM_STATE)
    cv=StratifiedKFold(n_splits=5,shuffle=True,random_state=RANDOM_STATE)
    rs=RandomizedSearchCV(xgb,pd,n_iter=80,scoring='roc_auc',cv=cv,n_jobs=-1,verbose=0,random_state=RANDOM_STATE)
